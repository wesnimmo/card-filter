...
export function CardDetail(props: {card: Card;}) {
  ...
  // State only matters for the eye-toggle in modal view
  const [showOverlay, setShowOverlay] = useState(true);

  return (
    <div className="not-last:group relative w-full overflow-hidden rounded-xl bg-black text-left shadow-lg transition-all duration-300 h-[485px] max-w-[395px]">
      {/* Vertical label strip: Thinner for grid */}
      <div className="absolute left-0 top-0 z-20 h-full w-6 bg-black/90">
        <div className="absolute left-1/2 top-6 -translate-x-1/2 font-bold tracking-[0.25em] text-[13px] text-zinc-400 opacity-80 [writing-mode:vertical-rl]">
      ...
      <div className="relative h-full ml-6">
        {/* Background image: Subtle zoom on hover for grid */}
        <div
          className="absolute inset-0 z-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110 brightness-90"
        ...
        />
          ...
          >
            ...
            <div className="absolute left-4 right-0 top-2 rounded-l-md bg-black/55 px-3 py-2 text-white backdrop-blur-[1px]">
              <h2 className="pr-10 text-[21px] font-bold leading-tight">{card.title}</h2>
              <div className="mt-1 flex flex-wrap gap-1">
                {card.categories.map((c) => (
                  <span key={c} className={`rounded-sm px-2 py-[2px] text-[11px] font-medium text-white ${CATEGORY_COLOR[c]}`}>
                    {CATEGORY_LABEL[c]}
                  </span>
                ))}
              </div>
            ...
            <div className="group/impact absolute bottom-5 right-3 h-[150px] w-[40%] overflow-hidden rounded-xl bg-white/45 p-3 text-[14px] font-light leading-snug text-zinc-900 shadow-sm transition hover:overflow-auto hover:shadow-md">
              <p><span className="font-bold">Impact:</span><br />{card.impact}</p>
...
function DataBlock({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md bg-black/55 px-3 py-2 text-[12px]">
      <div className="w-[40%] border-b-2 border-white font-semibold capitalize">{label}</div>
      <div className="mt-1 font-light capitalize">{value}</div>
    </div>
  );
}
